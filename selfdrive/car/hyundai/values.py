...
class HyundaiCanFDPlatformConfig(PlatformConfig):
  dbc_dict: DbcDict = field(default_factory=lambda: dbc_dict("hyundai_kia_generic", None))#dbc_dict: DbcDict = field(default_factory=lambda: dbc_dict("hyundai_canfd", None))
...
class CAR(Platforms):
  ...
  HYUNDAI_ELANTRA_2024 = HyundaiPlatformConfig(
    [HyundaiCarDocs("Hyundai Elantra 2024", video_link="https://youtu.be/_EdYQtV52-c", car_parts=CarParts.common([CarHarness.hyundai_k]))],
    CarSpecs(mass=2800 * CV.LB_TO_KG, wheelbase=2.72, steerRatio=12.9, tireStiffnessFactor=0.65),
    flags=HyundaiFlags.CHECKSUM_CRC8 | HyundaiFlags.CAMERA_SCC | HyundaiFlags.CANFD,
  )
